...
import { ResponseGetCampLatestList, ResponseGetCampWrapper } from '@/feature/camps/types/responses';
...
export async function GET(request: Request) {
  try {
    ...
    if (isNaN(pageNo) || pageNo < 1) {
      return NextResponse.json({ error: 'Invalid pageNo parameter' }, { status: 400 });
    }
    ...
    if (!res.ok) {
      throw new Error(`Failed to fetch camp latest list: ${res.status} ${res.statusText}`);
    }
    ...
    return NextResponse.json({ error: 'Failed to fetch camp latest list' }, { status: 500 });
  }
}
